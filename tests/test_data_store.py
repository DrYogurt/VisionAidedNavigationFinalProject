...
class TestVersionedDataStore(unittest.TestCase):
    ...
    def test_v9_cache_is_distinct_from_legacy_results(self):
        with tempfile.TemporaryDirectory() as directory:
            store = VersionedDataStore(directory)
            self.assertEqual(store.VERSION, "v9.0.0")
            self.assertFalse(store.exists(0, 2, 50, {"num_classes_in_model": 5}))
